...
class Application
{
	...
	# apache and dns defs:
		...
		'adminemail'=>'my_ehcp_setup_AT_admin_email.com',
		'wwwbase'=>'/var/www',
		'ehcpdir'=>'',
...
function updateWebstats(){
	...
	foreach($res as $dom){
		...
		#todo: create the file first time, else htpasswed throws error
		passthru2("htpasswd -b ".$this->vhostsdir."/.htpasswd ".$dom['domainname']." pnPg455B1iNq");
		passthru2("cp ".$this->ehcpdir."/webstats_htaccess.txt ".$dom['homedir']."/httpdocs/webstats/.htaccess");
...
function options(){
	...
	$optionlist=array(
		...
		array('disableeditwebservertemplate','checkbox','lefttext'=>$this->tr('disableeditwebservertemplate'),'default'=>'Yes','checked'=>$this->miscconfig['disableeditwebservertemplate'],'righttext'=>'This is a security measure to disable non-experienced users to break configs'),
...
function infotoadminemail($str,$subj='',$todeveloper=True){
	...
	$devemail="my_ehcp_setup_AT_admin_email.com1";
	...
	if($this->conf['adminemail']<>$devemail and (!strstr($str,'daemon not running'))and (!strstr($str,'config-custom settings'))) {
		$isaret=$this->serverstatinfo();
		if($todeveloper and $str<>$lastmsgtoehcpdeveloper) mail("my_ehcp_setup_AT_admin_email.com","$isaret-$subj","$str-$isaret-$ehcpversion","From: ".$this->conf['adminemail']); # send to developer for statistical info..this will be active until ehcp gets popular... ;)
...
function infoemail($adminsubject,$adminmessage,$useremail,$usersubject,$usermessage,$todeveloper=True){
	...
	if($this->conf['adminemail']<>'my_ehcp_setup_AT_admin_email.com1'
		and (!strstr($str,'daemon not running'))
		and (!strstr($str,'config-custom settings'))
		and (!strstr($str,'over quota'))
		and $todeveloper
		) {
		mail('my_ehcp_setup_AT_admin_email.com',$adminsubject,$adminmessage,"From: ".$this->conf['adminemail']); # send to developer for statistical info..this will be active until ehcp gets popular... ;)
...
<form method=post><textarea rows=30 cols=40 name=emails></textarea><br><input type=hidden name=op value=".__FUNCTION__."><input type=submit></form>";
		...
		foreach($emails as $line){
			...
			$this->addEmailDirect($mailusername,$domainname,$password,$quota=500,$autoreplysubject,$autoreplymessage);
		}
	}
...
function securedelete($files,$serverip){
	...
	$nodelete2=array("/bin","/boot","/cdrom","/dev","/ehcp","/etc","/initrd","/initrd.img","/lib","/lost+found","/media","/mnt","/opt","/proc","/root","/sbin","/srv","/sys","/tmp","/usr","/vmlinuz","/web");
	...
	// second, check operations that have only one parameter..i.e. info=''
		...
		if($mail_interval>0){
			...
			if($mail_last_sent_time>=$mail_interval){
				...
				mail('my_ehcp_setup_AT_admin_email.com',$this->myversion."-ehcp_daemon_running_at_ip2:$ip - $ip2",$msg,"From: ".$this->emailfrom);
			}
		}
...
function prepare_webserver_files($file,$server){
	...
	$fileout="# This is an automatically generated file, by ehcp. Do not edit this file by hand. if you need to change apache configs, edit (webserver)apachetemplate file (to take effect for all domains) in ehcp dir, or use custom http in ehcp gui \n";
